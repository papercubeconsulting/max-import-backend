...
const uploadImagesZip = async (reqParams, file, reqUser) => {
  ...
  try {
    ...
    const data = await new Promise((resolve, reject) => {
      stream
        ...
        .on('entry', async function(entry) {
          ...
          const validProduct = await validateProduct(fileName);
          const rowResponse = {
          ...
          if (fileType === 'File' && validProduct.valid) {
            ...
            entry.buffer().then(async buffer => {
              const base64Data = `data:image/jpeg;base64,${Buffer.from(
                buffer,
              ).toString('base64')}`;

              // TODO: ecapsulat this logic
              let updateData = {};
              if (validProduct.position == '1') {
                updateData = { imageBase64: base64Data };
              }
              if (validProduct.position == '2') {
                updateData = { secondImageBase64: base64Data };
              }
              if (validProduct.position == '3') {
                updateData = { thirdImageBase64: base64Data };
              }
            ...
            rowResponse.reason =
              'Código de producto no existe o número de imagen incorrecto';
            entry.autodrain();
          }
    ...
    return setResponse(200, 'Images uploaded and unzipeped successfully', {
