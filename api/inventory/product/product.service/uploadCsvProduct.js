...
const checkCategory=async(Category, categoryName, parent, parentKey, depth)=>{
    let category;
    let categoryId;
    ...
    if (parentKey && parent && parent.id){ firstQuery[parentKey] = parent.id};
...
const uploadCsvProduct = async(reqParams, file, reqUser)=>{
    ...
    for(let i=0; i<bulkProducts.length; i++){
        ...
        let familyName=bulkProducts[i].familia;
        let subfamilyName=bulkProducts[i].subFamilia;
        let elementName=bulkProducts[i].elemento;
        let modelName = bulkProducts[i].modelo;
        let compatibility= bulkProducts[i].compatibilidad;
        let tradename = bulkProducts[i].nombreComercial;
        let suggestedPrice= parseInt(bulkProducts[i].precio);
        let providerName =bulkProducts[i].proveedor;
        ...
        console.log(`Item ${i+1}: family: ${familyName}, subFamily:${subfamilyName} proveedor: ${bulkProducts[i].proveedor}`);

        
        let itemProductDraft = {
            familyName,
            subfamilyName,
            elementName,
            modelName
        ...
        let family = await checkCategory(Family,familyName,null, null,0);
        ...
        let subfamily = await checkCategory(Subfamily, subfamilyName, family.next, 'familyId',1,);
        ...
        let element = await checkCategory(Element,elementName, subfamily.next, 'subfamilyId',2,);
        ...
        let model = await checkCategory(Model, modelName, element.next,'elementId',3,);
        ...
        let validation = {family, subfamily, element, model};
        let categories = await createCategories(itemProductDraft, validation);

        console.log(categories.model.id);
        let product = await Product.findOne({where:{modelId:categories.model.id}});
    ...
    return setResponse(200, 'Products upload', {fields:columnsOutput, data:row});
}
